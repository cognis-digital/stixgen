...
**stixgen** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `stixgen` emits JSON findings, pipe them straight to a destination — `--dry-run`
...
works whatever `stixgen` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="stixgen") for rec in records]   # records = your JSON output
