...
[![PyPI](https://img.shields.io/pypi/v/cognis-stixgen.svg)](https://pypi.org/project/cognis-stixgen/)
[![CI](https://github.com/cognis-digital/stixgen/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/stixgen/actions)
...
STIXGEN exists for one job — build stix 2.1 bundles from a list of iocs/observables — and does it without a SaaS bill or heavyweight setup.
...
```bash
stixgen --version
stixgen scan .                      # scan the current project
stixgen scan . --format json
stixgen scan . --fail-on high       # non-zero exit for CI gates
stixgen mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **stix2**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`stixgen` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
