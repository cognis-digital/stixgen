# Comprehensive installer for cognis-digital/stixgen (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# stixgen is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "stixgen"
$Url  = "git+https://github.com/cognis-digital/stixgen.git"
$Git  = "https://github.com/cognis-digital/stixgen.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: stixgen"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: stixgen"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: stixgen"; exit 0 }
}
