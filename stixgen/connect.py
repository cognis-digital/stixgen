"""Native cognis-connect emit for stixgen — forward findings to any platform.

Maps stixgen's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    stixgen ... --format json | stixgen-emit --to stix
    stixgen-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a stixgen record toward the Finding contract before normalize().
